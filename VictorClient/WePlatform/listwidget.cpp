...
alistwidget::alistwidget(QWidget *parent):QListWidget(parent)
{
}
void alistwidget::contextMenuEvent ( QContextMenuEvent * event )
{
...
void alistwidget::mousePressEvent(QMouseEvent * event)
{
    if(this->itemAt(event->pos()))
    {
        QListWidgetItem *item=this->itemAt(event->pos());
...
void alistwidget::actionone()
{
    action1->clicked();
}

void alistwidget::actiontwo()
{
    action2->clicked();
}
